...
{
   StringCchPrintf(szLine, cchMaxLen, TEXT("%p\t%s\t%12u\t"), pVMQ->pvRgnBaseAddress, GetMemStorageText(pVMQ->dwRgnStorage), pVMQ->RgnSize);

   if (pVMQ->dwRgnStorage != MEM_FREE) 
   {
      StringCchPrintf(_tcschr(szLine, 0), cchMaxLen - _tcslen(szLine), TEXT("%u\t"), pVMQ->dwRgnBlocks);
      GetProtectText(pVMQ->dwRgnProtection, _tcschr(szLine, 0), cchMaxLen - _tcslen(szLine), FALSE);
   } 
   else 
   {
   ...
   if ((pVMQ->dwRgnStorage != MEM_FREE) && (pVMQ->pvRgnBaseAddress != NULL)) 
   {
      ...
      else 
      {
         ...
         // 这不是一个模块;查看它是否是内存映射文件
         DWORD dwLen = GetMappedFileName(hProcess, pVMQ->pvRgnBaseAddress, szLine + cchLen, cchMaxLen - cchLen);
         if (dwLen == 0) 
         {
            // GetMapped文件名在字符串失败时修改字符串
            szLine[cchLen] = 0;
         }
      }
   }
...
int WINAPI _tWinMain(HINSTANCE hInstExe, HINSTANCE, PTSTR pszCmdLine, int) {
   ...
   if (g_dwProcessId == 0) 
   {
      ...
      MessageBox(NULL, TEXT("No Process Id passed on command line. Defaulting to this process"), TEXT("VMMap"), MB_ICONINFORMATION);
   }
